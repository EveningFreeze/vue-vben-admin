  ...
  <div class="p-6 h-full bg-[#f8fafc] dark:bg-[#09090b] overflow-y-auto custom-scrollbar">
    ...
    <div v-if="!isReviewing" class="animate-fade-in">
      <div class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="md:col-span-2 bg-gradient-to-r from-blue-600 to-blue-700 rounded-3xl p-8 text-white shadow-xl flex justify-between items-center relative overflow-hidden">
          <div class="relative z-10">
            <h1 class="text-2xl font-bold mb-2">词汇能力测评</h1>
            <p class="text-blue-100 opacity-90 text-sm">今日已坚持练习 15 分钟，继续加油！</p>
            <button 
              @click="startReview" 
              class="mt-4 px-6 py-2 bg-white text-blue-600 rounded-full font-bold text-sm shadow-lg hover:bg-blue-50 transition-all active:scale-95"
            >
              开始今日回顾
            </button>
          </div>
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-xl font-extrabold text-gray-800 dark:text-zinc-100 flex items-center gap-3">
            <span class="w-2 h-6 bg-blue-500 rounded-full shadow-lg"></span>
            阶段测试
          </h2>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-5">
          <div v-for="test in stageTests" :key="test.title" 
               ...
               class="bg-white dark:bg-[#18181b] p-6 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm hover:border-blue-400 hover:shadow-lg transition-all cursor-pointer group relative">
            ...
            <h3 class="text-base font-black text-gray-900 dark:text-zinc-100 mb-2 group-hover:text-blue-600">
              {{ test.title }}
            </h3>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-6">
        <div class="bg-white dark:bg-[#18181b] rounded-3xl p-7 border border-gray-200 dark:border-zinc-800 shadow-sm">
          ...
          <div class="flex flex-col sm:flex-row gap-4">
            <div v-for="item in vocabTests" :key="item.title" 
                 class="flex-1 p-5 bg-slate-50 dark:bg-zinc-900/50 rounded-2xl border border-transparent hover:border-blue-200 transition-all cursor-pointer flex items-center gap-4">
        ...
        <div class="bg-white dark:bg-[#18181b] rounded-3xl p-7 border border-gray-200 dark:border-zinc-800 shadow-sm flex flex-col justify-center">
          <div class="flex justify-between items-end mb-5">
            <div>
              <div class="text-[11px] font-black text-blue-400 uppercase tracking-widest">Efficiency</div>
    ...
    <div v-else class="animate-fade-in max-w-2xl mx-auto py-10">
      <div class="flex items-center justify-between mb-8">
        <button @click="isReviewing = false" class="flex items-center gap-2 text-gray-500 hover:text-blue-600 transition-colors">
          <span>← 返回测评中心</span>
        </button>
        <div class="text-sm font-bold text-blue-600 bg-blue-50 px-4 py-1 rounded-full">
          进度：{{ wordIndex + 1 }} / {{ reviewList.length }}
        </div>
      </div>

      <div class="bg-white dark:bg-[#18181b] rounded-[40px] p-12 shadow-2xl border border-blue-100 dark:border-zinc-800 text-center relative">
        <div class="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 bg-blue-600 rounded-3xl shadow-xl flex items-center justify-center text-4xl text-white">
          📖
        </div>
        ...
        <div class="mt-4">
          ...
          <div class="flex justify-center items-center gap-3 mb-10">
            <span class="text-xl text-blue-500 font-mono italic">[{{ currentWord.phonetic }}]</span>
            <button @click="playAudio" class="p-2 hover:bg-blue-50 rounded-full transition-colors text-xl">🔊</button>
          </div>

          <div v-if="showAnswer" class="animate-slide-up">
            <div class="p-8 bg-blue-50/50 dark:bg-blue-500/5 rounded-[30px] text-left border border-blue-100 dark:border-blue-900/30 mb-8">
              <div class="text-sm font-black text-blue-400 uppercase mb-2 tracking-widest">Meaning</div>
              <div class="text-2xl font-bold text-gray-800 dark:text-zinc-100 mb-4">{{ currentWord.meaning }}</div>
              <div class="text-sm font-black text-blue-400 uppercase mb-2 tracking-widest">Example</div>
              <div class="text-base text-gray-600 dark:text-zinc-400 italic">"{{ currentWord.example }}"</div>
            </div>
            
            <button 
              @click="handleNext" 
              class="w-full py-5 bg-blue-600 text-white rounded-2xl font-bold text-lg shadow-xl hover:bg-blue-700 hover:shadow-blue-500/30 transition-all active:scale-95"
            >
          ...
          <button 
            ...
            class="w-full py-5 border-2 border-dashed border-blue-200 text-blue-600 rounded-2xl font-bold text-lg hover:bg-blue-50 transition-all"
          >
            点击查看词义解析
          </button>
        </div>
...
const stageTests = [
  ...
  { title: '看中选英', desc: '强化中文释义到英文的转化，提升口语表达', icon: '💡', iconBg: 'bg-zinc-100 dark:bg-zinc-1000/10', tag: 'DIAMOND', path: '' },
...
const progress = [
  ...
  { label: '进阶词汇 (Level 3-4)', val: 72, color: 'bg-blue-500' },
